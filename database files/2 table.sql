...
GO
 ...
 create table dbo.RUser(
    ...
    FirstName varchar(25) not null CONSTRAINT ck_User_First_name_cannot_be_blank CHECK(FirstName > ''),
    LastName varchar(25) not null CONSTRAINT ck_User_Last_name_cannot_be_blank CHECK(LastName > ''),
...
create table dbo.Measurement(
    ...
    MeasurementDesc VARCHAR(25) not null 
        CONSTRAINT ck_Measurement_Desck_cannot_be_blank CHECK(MeasurementDesc > '')
...
create table dbo.Course(
    ...
    CourseDesc VARCHAR(25) not null
        CONSTRAINT ck_ourse_Desck_cannot_be_blank CHECK(CourseDesc > '')
        CONSTRAINT u_Course_Desc unique,
    CourseOrder INT not null CONSTRAINT ck_Course_Order_must_be_greater_than_zero CHECK(CourseOrder > 0)
)
GO
...
create table dbo.Cuisine(
    ...
    CuisineDesc VARCHAR(25) not null 
        CONSTRAINT ck_Cuisine_Desck_cannot_be_blank CHECK(CuisineDesc > '')
...
create table dbo.RecipeIngredient(
    ...
    Amount decimal(5,2) not NULL constraint ck_RecipeIngredient_amount_must_be_greater_than_zero check(Amount > 0),
    RecipeIngredientOrder INT NOT NULL CONSTRAINT ck_RecipeIngredient_Order_must_be_greater_than_zero CHECK(RecipeIngredientOrder > 0),
...
create table dbo.Direction(
    ...
    StepDesc VARCHAR(200) NOT NULL CONSTRAINT ck_Direction_step_desck_cannot_be_blank CHECK(StepDesc > ''),
    DirectionOrder INT NOT NULL CONSTRAINT ck_Direction_Order_must_be_greater_than_zero CHECK(DirectionOrder > 0),
...
create table dbo.CookBook(
    ...
    CookBookPrice DECIMAL(5,2) NOT NULL CONSTRAINT ck_CookBook_price_must_be_greater_than_zero CHECK(CookBookPrice > 0),
    IsActive bit NOT NULL DEFAULT 1,
    DOE DATE NOT NULL DEFAULT GETDATE()
)
GO
...
create table dbo.CookBookRecipe(
    ...
    CookBookRecipeOrder INT not null CONSTRAINT ck_CookBookRecipe_ordermust_be_greater_than_zero CHECK(CookBookRecipeOrder > 0),
